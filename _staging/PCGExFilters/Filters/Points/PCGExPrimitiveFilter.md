---
icon: filter
description: 'Filter : Primitive Overlap - Tests point OBB overlap against primitive component collision.'
---

# Filter : Primitive Overlap

Creates a filter definition that tests point OBB overlap against primitive component collision.
...
4. **Result**: The point passes if any primitive overlap is detected. If `Invert` is enabled, the logic flips — points pass only when they overlap *no* primitives.
...
If enabled, inverts the result of the test — points that do *not* overlap any primitive will pass instead.
...
<!-- VERIFICATION REPORT
Node-Specific Properties: 2 documented (BoundsSource, bInvert from FPCGExPrimitiveFilterConfig)
Inherited Properties: Referenced to UPCGExFilterProviderSettings
Inputs: Primitives (custom pin)
Outputs: Filter (inherited from provider)
Nested Types: FPCGExPrimitiveFilterConfig (documented inline), EPCGExPointBoundsSource (external enum, referenced)
-->
