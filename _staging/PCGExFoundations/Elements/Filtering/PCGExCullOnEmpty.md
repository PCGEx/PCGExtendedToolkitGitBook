---
icon: cog
description: 'Cull On Empty - Deactivates output pin if all inputs are empty or missing.'
---

# Cull On Empty

Deactivates output pin if all inputs are empty or missing.
...
3. **Deactivate If Empty**: If *no* inputs contain valid data, the output pin is deactivated. This signals the PCG graph that downstream execution should be skipped.
...
<!-- VERIFICATION REPORT
Node-Specific Properties: 0 (node has no UPROPERTYs)
Inherited Properties: N/A (base UPCGExSettings — no shared settings relevant)
Inputs: In (dynamic, any type)
Outputs: Out (dynamic, any type, can be deactivated)
Nested Types: None
-->
